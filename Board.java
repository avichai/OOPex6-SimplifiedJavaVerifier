...
/**
 * The Board class represents a board of the Nim game. The board can be of a 
 * variable size. In this implementation 
 * it only has 4 rows, 7 sticks in the first row, and 5,3 and 1 sticks in the 
 * following rows. A board object is 
 * disposable, meaning that the moves performed on the board are not reversible, 
 * and if a "clean" board is required, 
...
public class Board {
	...
	private static final int[][] boardTemplate = 
	...
	private static final int NUM_OF_ELEMENTS = NUM_OF_ROWS*
									(1+MAX_NUM_OF_STICKS_IN_ROW)/2;	
	private static final int INITIAL_MARKED_STICKS = 0;
	 ...
	 */
	public Board(){		
		numberOfMarkedSticks = INITIAL_MARKED_STICKS;
	...
	public String toString(){
		/*
		 * When we implement a public method named 'toString()' 
		 * (exactly this name!) which returns a String for some class, then 
		 * when we send an object of this class to methods like 
		 * System.out.print() or println(), they will print this output String. 
		 * If the object has no such method, print() and println() will print 
		 * using the template class_name@hashCodeIdentifier, which in this case 
		 * will look something like oop.ex1.Board@55f96302
		 */
		String output = "";
	 ...
	 * In case the move is illegal the board is not changed and an appropriate 
	 * error code is returned:
	 * If the given coordinates exceed the boundaries of the board, -1 is 
	 * returned. 
	 * If the current move overlaps with previously marked sticks, 0 is 
	 * returned.
	 * If the move is legal, the board changes accordingly, the number of 
	 * marked sticks in the "numMarked" is updated,
	 * 1 is returned.
	 * @param move the move to perform
	 * @return 1 if the move was legal, 0 and -1 if the move is not legal 
	 * (details above).
	 */
	public int markStickSequence(Move move){
		...
		if( (leftBound < 1) || (leftBound > rightBound) || rowNumber <1 
				|| rowNumber>NUM_OF_ROWS || (rightBound>
												gameBoard[rowNumber-1].length) )
			return -1;
	...
	/**
	 * Returns the total number of sticks (marked and unmarked) in the row. 
	 * A legal input to this method is
	 * an integer number between 1 and the output of "getNumberOfRows()". 
	...
	/**
	 * Given an index to the stick position (row and number in row - 
	 * counting from the left side), 
	 * this method returns true if the stick is unmarked, and false if it is 
	 * marked, or if the input is out of bounds. 
	 */
